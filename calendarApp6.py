        ...
        for row, week in enumerate(month_data):
            for col, day in enumerate(week):
                if day.month != list(calendar.month_name).index(self.current_month):
                    button = tk.Button(self.frame, text="", font=("Arial", 12), state=tk.DISABLED)
                else:
                    button = tk.Button(self.frame, text=day.day, font=("Arial", 12),
                                       command=lambda d=day: self.show_day_info(d))
                button.grid(row=row + 1, column=col, padx=10, pady=10)

    def show_day_info(self, day):
        # Display information about the selected day
        info = f"You clicked on {day.strftime('%A, %B %d, %Y')}"
        tk.messagebox.showinfo(title="Day Information", message=info)
